...
export class RouteMatcher {
  ...
  public match(path: string): RouteMatch | null {
    ...
    for (const segment of this.#segments) {
      ...
      if (segment.type === 'static') {
        if (segment.value !== pathSegments[consumedSegments]) {
          return null;
        }
        consumedSegments++;
  ...
  /**
   * Checks if this route pattern is a wildcard route (ends with /*).
   */
  public isWildcard(): boolean {
    return this.#segments.length > 0 && this.#segments[this.#segments.length - 1]?.type === 'wildcard';
  }

  /**
   * Checks if this route pattern is an index route (exactly '/').
   */
